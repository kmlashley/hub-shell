...
export default function ProjectDetailPage({
  ...
  async function addDeliverable() {
    ...
    const next = [...deliverables, { id: crypto.randomUUID(), text: newDeliverable.trim(), done: false }];
  ...
  async function toggleDeliverable(itemId: string) {
    const next = deliverables.map((d) => d.id === itemId ? { ...d, done: !d.done } : d);
    setDeliverables(next);
    await persistJson(next, links);
  }
  ...
  async function addLink() {
    ...
    const next = [...links, { id: crypto.randomUUID(), label: newLinkLabel.trim() || newLinkUrl.trim(), url: newLinkUrl.trim() }];
    setLinks(next);
    setNewLinkLabel("");
    setNewLinkUrl("");
  ...
  if (!project) {
    return (
      <div className="max-w-3xl mx-auto text-center py-20">
        ...
        <Link href="/projects" className="text-sm text-primary mt-2 inline-block hover:underline">
          ← Back to projects
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-xs text-muted mb-6">
        ...
        {" › "}
        <Link href="/projects" className="hover:text-dark transition-colors">Projects</Link>
        {" › "}
